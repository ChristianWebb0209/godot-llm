...
## Optional popup settings UI: Display (text size, word wrap) and AI (provider, API key, base URL).
## Persists via GodotAISettings; notifies parent to apply and save.
## Main Settings UI is the dock's Settings tab (see settings_tab.gd).
