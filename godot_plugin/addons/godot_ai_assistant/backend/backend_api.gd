...
## Backend API: query for tools, log edit events, post lint fix. Uses BackendClient and ContextPayload.
...
func query_backend_for_tools(
	...
	var endpoint := _dock.rag_service_url + "/query"
	...
	if settings:
		...
		if settings.selected_model.length() > 0:
			payload["model"] = settings.selected_model
		if settings.openai_base_url.length() > 0:
			payload["base_url"] = settings.openai_base_url

	var body := JSON.stringify(payload)
	var data = await GodotAIBackendClient.query_json(_dock, endpoint, HTTPClient.METHOD_POST, body)
