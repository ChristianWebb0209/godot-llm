"""
Colab training script for Composer v2 LoRA/QLoRA fine-tuning.

This trains only on:
  fine_tuning/data/composer_v2/train.jsonl
  fine_tuning/data/composer_v2/val.jsonl

It assumes the composer_v2 dataset already uses the Composer v2 contract:
...
def main() -> None:
    if not COMPOSER_TRAIN.exists() or not COMPOSER_VAL.exists():
        raise SystemExit(
            f"Missing composer_v2 datasets. Expected:\n- {COMPOSER_TRAIN}\n- {COMPOSER_VAL}"
        )
