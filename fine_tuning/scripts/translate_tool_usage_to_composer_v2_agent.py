#!/usr/bin/env python3
"""
Translate fine_tuning/data/tool_usage/* into Composer v2 AGENT-mode training records.
...
Output JSONL format:
  {"messages": [
      ...
      {"role":"assistant","content": <original assistant (must contain <tool_call> blocks)>}
   ]}
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Translate tool_usage JSONL -> Composer v2 AGENT dataset")
    parser.add_argument(
        "--input-train",
        type=str,
        default=str(REPO_ROOT / "fine_tuning" / "data" / "tool_usage" / "train.jsonl"),
    )
    parser.add_argument(
        "--input-val",
        type=str,
        default=str(REPO_ROOT / "fine_tuning" / "data" / "tool_usage" / "val.jsonl"),
    )
    parser.add_argument(
        "--output",
        type=str,
        default=str(REPO_ROOT / "fine_tuning" / "data" / "composer_v2" / "agent_from_tool_usage.jsonl"),
    )
    ...
    inputs = [Path(args.input_train)]
    if args.input_val:
        inputs.append(Path(args.input_val))
